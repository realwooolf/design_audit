...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (diffImage) {
      ...
      parts.push({ text: '【第三张图：叠加对比图】这是一个分析辅助工具图，不是真实界面截图。它是将设计稿以 50% 透明度叠加在开发稿上生成的。重要：不要把叠加图中的重影、半透明、双影描述为界面问题，这些是叠加工具本身的视觉效果。正确用法：用叠加图定位哪些区域存在差异（有重影的地方 = 设计稿与开发稿在该位置不一致），然后回到第一张和第二张图去观察和描述具体问题。重影形状参考：四角弧形重影 → 圆角差异；元素间条状重影 → 间距差异；色块整体重影 → 颜色差异；文字双影 → 字号或内容差异。间距和圆角问题宁可多报不要漏报。' });
