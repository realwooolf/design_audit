...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (diffImage) {
      ...
      parts.push({ text: '【第三张图：叠加对比图】设计稿以 50% 透明度叠加在开发稿上。有重影或模糊边缘的地方就是两图存在差异的位置。根据重影的形状判断问题类型：元素四角有弧形重影 → 圆角大小不一致；元素之间有条状重影 → 间距或 padding 不一致；整个色块有重影 → 颜色差异；文字有双影 → 字号或内容差异。间距和圆角问题宁可多报不要漏报。' });
